...
if($method === 'POST') {
    ...
    try {
        success(encrypt($requestData['folder'], '4Ow0TiivfwuADqi5uCKbDh3P9OPUTeF+yjT+exuadSU='));
    }
    catch(Exception $e) {
        fail($e.getMessage());
    }
...
document.querySelector("#encrypt").addEventListener("click", async function(event) {
  ...
  try {
    document.querySelector("#encrypted_link").value = "YWhtbmVvLmdpdGh1Yi5pby9nZHJpdmUtZmlsZS1jb3BpZXI=." + (await encryptFolder());
  }
  catch(e) {
    alert(e.message);
  }
